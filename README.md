...
This repo experiment builds on the concept used my many developers to group @Inputs into one or two big ones for easier maintenance ([See this repo by Tim Deschryver for a similar approach](https://github.com/timdeschryver/ngrx-family-grocery-list/blob/master/src/app/groceries/pages/grocery-family-member-page.component.ts)). The next step to me seems to find the easiest way to group @Outputs together into one. [See the bus-demo app for how I've achieved that](apps/bus-demo/src/app/book-manager/book-manager.component.html).
